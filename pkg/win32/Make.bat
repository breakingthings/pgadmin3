...
%WIXDIR%\candle -nologo -dWXDIR="%WXWIN%" -dPGDIR="%PGDIR%" -dOPENSSL="%OPENSSL%" -dPGBUILD="%PGBUILD%" -dBUILDTREE="%BUILDTREE%" -dBRANDED=%BRANDED% -dBRANDINGDIR="%BRANDINGDIR%" -dAPPVENDOR="%APPVENDOR%" -dAPPNAME="%APPNAME%" -dAPPKEYWORDS="%APPKEYWORDS%" -dAPPCOMMENTS="%APPCOMMENTS%" -dAPPDESCRIPTION="%APPDESCRIPTION%" -dAPPVERSION="%1" -dSYSTEM32DIR="%SystemRoot%\System32" -dPFILESDIR="%ProgramFiles%" -arch x64 src/pgadmin3.wxs 
IF ERRORLEVEL 1 GOTO ERR_HANDLER

%WIXDIR%\light -sice:ICE03 -sice:ICE25 -sice:ICE82 -sw1101 -nologo -ext WixUIExtension -cultures:en-us -out pgadmin3_x64.msi pgadmin3.wixobj
IF ERRORLEVEL 1 GOTO ERR_HANDLER
